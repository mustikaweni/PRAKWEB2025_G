...
<div class="relative overflow-x-auto bg-neutral-primary-soft shadow-xs rounded-base border border-default">
    <table class="w-full text-sm text-left rtl:text-right text-body">
        <thead class="text-xs text-sm text-body-bg-neutral-secondary-soft border-b border-default">
            <tr>
...
<div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
    {{-- Search Form --}}
    <form method="GET" action="{{ route('dashboard.index') }}" class="flex max-w-md">
        ...
        <div class="relative">
            ...
            <button type="submit" class="absolute end-1.5 bottom-1.5 text-white bg-brand hover:bg-brand-strong box border-default rounded-base focus:ring-4 focus:ring-brand-medium shadow-xs font-medium leading-5 border-transparent focus:ring-4 focus:outline-none focus:ring-brand-medium rounded-base text-xs px-3 py-1.5">Search</button>
        </div>
    </form>
...
{{-- 3. Pagination --}}
    ...
    <div class="py-4 px-6 border-t border-gray-200">
        <nav aria-label="Page navigation" class="flex space-x-2 text-sm">
            <ul class="flex">
                {{-- Previous Button --}}
                ...
                <li>
                    ...
                    <a href="{{ $url }}" aria-current="page" class="flex items-center justify-center text-brand box-border border border-default-medium hover:bg-default-medium hover:text-brand font-medium text-sm w-10 h-10 focus:outline-none">{{ $page }}</a>
                    @else
